...
namespace HHCSService
{
    ...
    public class HHCSService : System.Web.Services.WebService
    {
        ...
        [WebMethod]
        public DataSet GetFoodExchangeData(int id)
        {
            try
            {
        ...
        /// <returns></returns>
        [WebMethod]
        public DataSet GetFoodData(string search_query)
        {
            try
            {
        ...
        /// <returns></returns>
        [WebMethod]
        public DataSet GetData(string tableName, string email, string password)
        {
            try
            {
                var mySQLConn = new MySqlConnection(EncodeHelper.Base64Decode(remoteAccess));
        ...
        /// <returns></returns>
        [WebMethod]
        public string[] SynchonizeData(string email, string password, List<TEMP_DiabetesTABLE> tempDiabetes, List<TEMP_KidneyTABLE> tempKidney, List<TEMP_PressureTABLE> tempPressure)
        {
            object result = null;
            ...
            try
            {
                ...
                if (ComparePassword(password, (string)mySQLCommand.ExecuteScalar()))
                {
                    ...
                    mySQLCommand.CommandText = $"SELECT ud_id FROM UserTABLE WHERE ud_email = '{email}'";
